import React from 'react'

export default function Container3() {
    return (
        <section className=" max-w-md mx-4 sm:max-w-6xl sm:mx-auto mt-32" id='skills'>
            <h1 className='border-b-4 inline-block border-blue-500 font-bold text-3xl pb-1 text-gray-700 '>My</h1><span className='pb-1 border-b-4 inline-block border-gray-200 font-bold text-3xl text-gray-700 pl-1.5'>Skills</span>
            <div className="grid grid-cols-1 sm:grid-cols-2 sm:gap-4 mt-9">
                <div className="col-span-1">
                    <div className="bg-white p-2">
                        <div className='flex justify-between around'>
                            <h2 className="text-md font-bold mb-1 text-gray-600">Javascript / Typescript</h2>
                            <h2 className='text-md font-bold mb-1 text-gray-500'>95%</h2>
                        </div>
                        <div className="w-full bg-white border-gray-200 border-2 rounded-full h-3 py-[2px] px-[2px] dark:bg-gray-700">
                            <div className="bg-blue-500 h-full rounded-full w-[95%]"></div>
                        </div>

                    </div>
                    <div className="bg-white p-2 mt-2">
                        <div className='flex justify-between around'>
                            <h2 className="text-md font-bold mb-1 text-gray-600">Microcontroller Programming</h2>
                            <h2 className='text-md font-bold mb-1 text-gray-500'>90%</h2>
                        </div>
                        <div className="w-full bg-white border-gray-200 border-2 rounded-full h-3 px-[2px] py-[2px] dark:bg-gray-700">
                            <div className="bg-blue-500 h-full rounded-full w-[90%]"></div>
                        </div>
                    </div>
                    <div className="bg-white p-2 mt-2">
                        <div className='flex justify-between around'>
                            <h2 className="text-md font-bold mb-1 text-gray-600">CAD Design</h2>
                            <h2 className='text-md font-bold mb-1 text-gray-500'>85%</h2>
                        </div>
                        <div className="w-full bg-white border-gray-200 border-2 rounded-full h-3 px-[2px] py-[2px] dark:bg-gray-700">
                            <div className="bg-blue-500 h-full rounded-full w-[85%]"></div>
                        </div>
                    </div>
                    
                    <div className="bg-white p-2 mt-2">
                        <div className='flex justify-between around'>
                            <h2 className="text-md font-bold mb-1 text-gray-600">Next js 13</h2>
                            <h2 className='text-md font-bold mb-1 text-gray-500'>85%</h2>
                        </div>
                        <div className="w-full bg-white border-gray-200 border-2 rounded-full h-3 px-[2px] py-[2px] dark:bg-gray-700">
                            <div className="bg-blue-500 h-full rounded-full w-[85%]"></div>
                        </div>
                    </div>
                    <div className="bg-white p-2 mt-2">
                        <div className='flex justify-between around'>
                            <h2 className="text-md font-bold mb-1 text-gray-600">UI/UX design</h2>
                            <h2 className='text-md font-bold mb-1 text-gray-500'>75%</h2>
                        </div>
                        <div className="w-full bg-white border-gray-200 border-2 rounded-full h-3 px-[2px] py-[2px] dark:bg-gray-700">
                            <div className="bg-blue-500 h-full rounded-full w-[75%]"></div>
                        </div>
                    </div>
                </div>
                <div className="col-span-1">
                    <div className="bg-white p-2">
                    <div className='flex justify-between around'>
                            <h2 className="text-md font-bold mb-1 text-gray-600">HTML / CSS</h2>
                            <h2 className='text-md font-bold mb-1 text-gray-500'>100%</h2>
                        </div>
                        <div className="w-full bg-white border-gray-200 border-2 rounded-full h-3 px-[2px] py-[2px] dark:bg-gray-700">
                            <div className="bg-blue-500 h-full rounded-full w-[100%]"></div>
                        </div>
                    </div>
                    <div className="bg-white p-2 mt-2">
                        <div className='flex justify-between around'>
                            <h2 className="text-md font-bold mb-1 text-gray-600">Python</h2>
                            <h2 className='text-md font-bold mb-1 text-gray-500'>95%</h2>
                        </div>
                        <div className="w-full bg-white border-gray-200 border-2 rounded-full h-3 px-[2px] py-[2px] dark:bg-gray-700">
                            <div className="bg-blue-500 h-full rounded-full w-[95%]"></div>
                        </div>
                    </div>
                    <div className="bg-white p-2 mt-2">
                        <div className='flex justify-between around'>
                            <h2 className="text-md font-bold mb-1 text-gray-600">Tailwind / Chakra</h2>
                            <h2 className='text-md font-bold mb-1 text-gray-500'>95%</h2>
                        </div>
                        <div className="w-full bg-white border-gray-200 border-2 rounded-full h-3 px-[2px] py-[2px] dark:bg-gray-700">
                            <div className="bg-blue-500 h-full rounded-full w-[95%]"></div>
                        </div>
                    </div>
                    <div className="bg-white p-2 mt-2">
                        <div className='flex justify-between'>
                            <h2 className="text-md font-bold mb-1 text-gray-600">Contentful</h2>
                            <h2 className='text-md font-bold mb-1 text-gray-500'>70%</h2>
                        </div>
                        <div className="w-full bg-white border-gray-200 border-2 rounded-full h-3 px-[2px] py-[2px] dark:bg-gray-700">
                            <div className="bg-blue-500 h-full rounded-full w-[70%]"></div>
                        </div>
                    </div>
                    <div className="bg-white p-2 mt-2">
                        <div className='flex justify-between'>
                            <h2 className="text-md font-bold mb-1 text-gray-600">Content Writing</h2>
                            <h2 className='text-md font-bold mb-1 text-gray-500'>50%</h2>
                        </div>
                        <div className="w-full bg-white border-gray-200 border-2 rounded-full h-3 px-[2px] py-[2px] dark:bg-gray-700">
                            <div className="bg-blue-500 h-full rounded-full w-[50%]"></div>
                        </div>
                    </div>
                </div>
            </div>


            {/* <div
                className=" flex items-center leading-8"
                id="skills"
            >
                <div className=" px-10 mt-9 mx-auto">
                    <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-16">
                        
                        <div className=" p-5 ">
                            <img
                                src="https://upload.wikimedia.org/wikipedia/commons/thumb/c/c3/Python-logo-notext.svg/1869px-Python-logo-notext.svg.png"
                                className="w-20 h-20  object-contain  hover:-translate-y-3 hover:scale-110 duration-700"
                            />
                            <p className="text-gray-700 text-center font-semibold mt-4 text-lg">Python</p>
                        </div>
                        <div className=" p-5 ">
                            <img
                                src="https://cdn-icons-png.flaticon.com/512/5968/5968292.png"
                                className="w-20 h-20  object-contain  hover:-translate-y-3 hover:scale-110 duration-700"
                            />
                            <p className="text-gray-700 text-center font-semibold mt-4 text-lg">Javascript</p>
                        </div>
                        <div className=" p-5 ">
                            <img
                                src="https://upload.wikimedia.org/wikipedia/commons/4/4c/Typescript_logo_2020.svg"
                                className="w-20 h-20  object-contain  hover:-translate-y-3 hover:scale-110 duration-700"
                            />
                            <p className="text-gray-700 text-center font-semibold mt-4 text-lg">Typescript</p>
                        </div>
                        <div className=" p-5 ">
                            <img
                                src="https://cdn-icons-png.flaticon.com/512/732/732212.png"
                                className="w-20 h-20  object-contain  hover:-translate-y-3 hover:scale-110 duration-700 "
                            />
                            <p className="text-gray-700 text-center font-semibold mt-4 text-lg">Html</p>
                        </div>
                        <div className="p-5">
                            <img
                                src="https://cdn-icons-png.flaticon.com/512/732/732190.png"
                                className="w-20 h-20 object-contain  hover:-translate-y-3 hover:scale-110 duration-700"
                            />
                            <p className="text-gray-700 text-center font-semibold mt-4 text-lg">CSS</p>
                        </div>
                        <div className=" p-5 ">
                            <img
                                src="https://upload.wikimedia.org/wikipedia/commons/thumb/a/a7/React-icon.svg/539px-React-icon.svg.png"
                                className="w-20 h-20  object-contain  hover:-translate-y-3 hover:scale-110 duration-700"
                            />
                            <p className="text-gray-700 text-center font-semibold mt-4 text-lg">React</p>
                        </div>
                        <div className=" p-5 ">
                            <img
                                src="https://d2nir1j4sou8ez.cloudfront.net/wp-content/uploads/2021/12/nextjs-boilerplate-logo.png"
                                className="w-20 h-20  object-contain  hover:-translate-y-3 hover:scale-110 duration-700"
                            />
                            <p className="text-gray-700 text-center font-semibold mt-4 text-lg">NextJS 13</p>
                        </div>
                        <div className=" p-5 ">
                            <img
                                src="https://upload.wikimedia.org/wikipedia/commons/thumb/d/d5/Tailwind_CSS_Logo.svg/2048px-Tailwind_CSS_Logo.svg.png"
                                className="w-20 h-20  object-contain  hover:-translate-y-3 hover:scale-110 duration-700"
                            />
                            <p className="text-gray-700 text-center font-semibold mt-4 text-lg">Tailwind</p>
                        </div>
                        <div className=" p-5 ">
                            <img
                                src="https://raw.githubusercontent.com/chakra-ui/chakra-ui/main/media/logo-colored@2x.png?raw=true"
                                className="w-20 h-20  object-contain  hover:-translate-y-3 hover:scale-110 duration-700"
                            />
                            <p className="text-gray-700 text-center font-semibold mt-4 text-lg">Chakra</p>
                        </div>
                        <div className=" p-5 ">
                            <img
                                src="https://cdn-icons-png.flaticon.com/512/919/919825.png"
                                className="w-20 h-20  object-contain  hover:-translate-y-3 hover:scale-110 duration-700"
                            />
                            <p className="text-gray-700 text-center font-semibold mt-4 text-lg">NodeJS</p>
                        </div>
                        <div className=" p-5 ">
                            <img
                                src="data:image/png;base64,"
                                className="w-20 h-20  object-contain  hover:-translate-y-3 hover:scale-110 duration-700"
                            />
                            <p className="text-gray-700 text-center font-bold mt-2">HackerRank</p>
                        </div>
                        <div className=" p-5 ">
                            <img
                                src="https://i.pinimg.com/originals/01/e5/00/01e500fca29c045d432b64f285f9c229.png"
                                className="w-20 h-20  object-contain  hover:-translate-y-3 hover:scale-110 duration-700"
                            />
                            <p className="text-gray-700 text-center font-semibold mt-4 text-lg">Git</p>
                        </div>
                        <div className=" p-5 ">
                            <img
                                src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSevjLMoY69RrmdOeKZONLsRySpvdkEwP-SaraZ3hc_8WFS0eR1ALIIL9xuP2_nWbRnawY&usqp=CAU"
                                className="w-20 h-20  object-contain  hover:-translate-y-3 hover:scale-110 duration-700"
                            />
                            <p className="text-gray-700 text-center font-semibold mt-4 text-lg">Github</p>
                        </div>
                    <div className=" p-5 ">
                        <img
                            src="https://upload.wikimedia.org/wikipedia/commons/thumb/8/87/Arduino_Logo.svg/720px-Arduino_Logo.svg.png"
                            className="w-20 h-20  object-contain  hover:-translate-y-3 hover:scale-110 duration-700"
                            />
                        <p className="text-gray-700 text-center font-semibold mt-4 text-lg leading-5">Arduino Programming</p>
                    </div>
                    < div className=" p-5 ">
                        <img
                        src="https://tech.coe.drexel.edu/images/software/solidworks.jpg"
                        className="w-20 h-20  object-contain  hover:-translate-y-3 hover:scale-110 duration-700"
                        />
                        <p className="text-gray-700 text-center font-semibold mt-4 text-lg">Solidworks</p>
                    </div> 
                    </div>
                </div>
            </div>  */}
        </section>
    )
}
